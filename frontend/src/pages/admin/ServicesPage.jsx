...
const emptyForm = { vendor_id: '', service_category_id: '', name: '', name_it: '', description: '', description_it: '', price: '', duration_minutes: 30 }
...
function ServicesPage() {
  ...
  useEffect(() => {
    api.get('/admin/vendors', { params: { page: 1 } })
      ...
      .catch(() => {})
    api.get('/admin/service-categories').then(r => setCategories(r.data.data || [])).catch(() => {})
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setShowModal(false); setForm(emptyForm); fetchServices()
    } catch (err) {
  ...
  const openBoostModal = (service) => {
    ...
    setBoostPrice(service.boost_price || '')
  }
  ...
  const handleBoost = async () => {
    ...
    try {
      ...
      await api.patch(`/admin/services/${boostTarget.id}`, payload)
      setBoostTarget(null); fetchServices()
    } catch (err) {
  ...
  const toggleActive = async (service) => {
    try {
      await api.patch(`/admin/services/${service.id}`, { is_active: !service.is_active })
      fetchServices()
    } catch (err) {
      alert(err.response?.data?.message || 'Failed')
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="admin-page-heading">Service Management</h2>
          <p className="admin-page-subheading">View, toggle, and remove vendor services</p>
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
            {boostTarget.is_boosted ? (
              ...
              <>
                <h3 className="text-base font-bold text-slate-900 mb-1">Boost Service</h3>
                  ...
                  {boostTiers.filter(t => t.is_active).map(t => (
                    <label key={t.id} className={`flex items-center justify-between gap-3 p-3 rounded-xl border-2 cursor-pointer transition-colors ${
                      ...
                      <div className="flex items-center gap-2">
                        <input type="radio" name="admin_boost_tier_s" className="accent-amber-500" checked={selectedTier === t.id} onChange={() => setSelectedTier(t.id)} />
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            <h3 className="text-base font-bold text-slate-900 mb-4">Add Service for Vendor</h3>
            ...
            <div className="flex gap-2 mt-5">
              ...
              <button onClick={handleCreate} disabled={saving} className="flex-[2] py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 text-white text-sm font-semibold rounded-lg border-0 cursor-pointer transition-colors">{saving ? 'Creating…' : 'Create Service'}</button>
            </div>
          </div>
        </div>
      )}
              ...
              <tbody>
                {services.map((s) => (
                  <tr key={s.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                    ...
                    <td className="py-3 px-4 font-medium text-gray-900">{getLocalizedField(s, 'name', i18n.language)}</td>
                    <td className="py-3 px-4 text-gray-500">{getLocalizedField(s.vendor, 'name', i18n.language, s.vendor?.name || `Vendor #${s.vendor_id}`)}</td>
                    <td className="py-3 px-4 text-gray-500">{s.service_category?.name || 'Uncategorized'}</td>
                    <td className="py-3 px-4 text-gray-700">€{parseFloat(s.price).toFixed(2)}</td>
                    <td className="py-3 px-4 text-gray-500">{s.duration_minutes} min</td>
                    <td className="py-3 px-4">
                      <span className={`admin-status-chip ${s.is_active ? 'admin-chip-success' : 'admin-chip-danger'}`}>
                        {s.is_active ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td className="py-3 px-4">
                      {s.is_boosted ? (() => {
                        const spent = parseFloat(s.boost_budget_spent) || 0
                        const budget = parseFloat(s.boost_budget) || 0
                        const left = Math.max(0, budget - spent)
                        return (
                          <div>
                            <span className="text-xs font-semibold bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">Boosted</span>
                            <p className="text-xs text-gray-500 mt-1">€{left.toFixed(2)} left &middot; {s.boost_clicks || 0} clicks</p>
                          </div>
                        )
                      })() : (
                    ...
                    <td className="py-3 px-4 text-gray-400">{new Date(s.created_at).toLocaleDateString()}</td>
                    <td className="py-3 px-4">
                      <div className="flex gap-1.5">
                        <ActionIconButton
                          onClick={() => openBoostModal(s)}
                          label={s.is_boosted ? 'Remove boost' : 'Boost service'}
                          className={`admin-action-icon ${s.is_boosted ? 'bg-amber-500 hover:bg-amber-600' : 'bg-amber-400 hover:bg-amber-500'}`}
                        >
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>
                        </ActionIconButton>
                        <ActionIconButton
                          onClick={() => toggleActive(s)}
                          label={`${s.is_active ? 'Deactivate' : 'Activate'} service`}
                          className={`admin-action-icon ${s.is_active ? 'bg-yellow-600 hover:bg-yellow-700' : 'bg-green-600 hover:bg-green-700'}`}
                        >
                          {s.is_active ? <DeactivateIcon /> : <ActivateIcon />}
                        </ActionIconButton>
                        <ActionIconButton
