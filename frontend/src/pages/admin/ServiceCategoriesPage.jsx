...
export default function ServiceCategoriesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await api.put(`/admin/service-categories/${editing.id}`, payload)
      } else {
  ...
  const toggleActive = async (item) => {
    await api.put(`/admin/service-categories/${item.id}`, { is_active: !item.is_active }).catch(() => {})
    fetchItems()
  }

  const handleDelete = async (item) => {
    if (!confirm(`Delete category "${item.name}"?`)) return
    await api.delete(`/admin/service-categories/${item.id}`).catch(() => {})
    fetchItems()
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="admin-page-heading">{t('admin.serviceCategories')}</h2>
          <p className="admin-page-subheading">{t('adminCats.serviceSubtitle', 'Create and manage service category options')}</p>
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            <h3 className="text-base font-bold text-slate-900 mb-4">{editing ? t('adminCats.editServiceCat', 'Edit Service Category') : t('adminCats.addServiceCategory', 'Add Service Category')}</h3>
            {error && <div className="bg-rose-50 text-rose-700 text-sm px-3 py-2 rounded-lg mb-3">{error}</div>}
            <div className="space-y-3">
              <label className="flex flex-col gap-1 text-xs font-medium text-slate-600">
                {t('common.name')}
                <input className="border border-slate-200 rounded-lg px-3 py-2 text-sm" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Facials" />
              </label>
              <label className="flex flex-col gap-1 text-xs font-medium text-slate-600">
                {t('adminCats.slug', 'Slug')} ({t('auth.optional', 'optional')})
                <input className="border border-slate-200 rounded-lg px-3 py-2 text-sm" value={form.slug} onChange={e => setForm(f => ({ ...f, slug: e.target.value }))} placeholder="facials" />
              </label>
