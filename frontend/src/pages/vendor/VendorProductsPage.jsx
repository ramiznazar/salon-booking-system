...
const empty = { name: '', name_it: '', description: '', description_it: '', image_url: '', price: '', stock: 0, product_category_id: '', is_active: true }
...
export default function VendorProductsPage() {
  ...
  const fetchProducts = () => {
    setLoading(true)
    api.get(`/vendor/products?page=${page}`)
      .then(r => { setProducts(r.data.data?.data || []); setMeta(r.data.data?.meta || r.data.data || {}) })
  ...
  useEffect(() => {
    ...
    api.get('/product-categories').then(r => setCategories(r.data.data || [])).catch(() => {})
  }, [])

  useEffect(() => { fetchProducts() }, [page])

  const openAdd = () => { setForm(empty); setEditing(null); setShowForm(true); setError('') }
  const openEdit = (p) => { setForm({ name: p.name || '', name_it: p.name_it || '', description: p.description || '', description_it: p.description_it || '', image_url: p.image_url || '', price: p.price, stock: p.stock, product_category_id: p.product_category_id || '', is_active: p.is_active }); setEditing(p.id); setShowForm(true); setError('') }
  ...
  const handleSave = async () => {
    ...
    try {
      editing ? await api.put(`/vendor/products/${editing}`, payload) : await api.post('/vendor/products', payload)
  ...
  const handleDelete = async (id) => {
    ...
    await api.delete(`/vendor/products/${id}`).catch(() => {}); fetchProducts()
  }

  const toggleActive = async (p) => {
    await api.put(`/vendor/products/${p.id}`, { is_active: !p.is_active }).catch(() => {}); fetchProducts()
  }
  ...
  const handleBoost = async () => {
    ...
    try {
      await api.post(`/vendor/products/${boostTarget.id}/boost`, { boost_tier_id: selectedTier, budget })
      setBoostTarget(null); setBoostBudget(''); fetchProducts()
    } catch (err) {
  ...
  const maxProducts = activePlan?.plan?.max_products ?? null
  const usedProducts = usage?.products ?? 0
  const atLimit = maxProducts !== null && usedProducts >= maxProducts
  const nearLimit = maxProducts !== null && usedProducts >= maxProducts * 0.8
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-900">{t('vendor.products')}</h2>
          <p className="text-sm text-slate-500 mt-0.5">{t('vendorProducts.subtitle', 'Manage your product catalogue')}</p>
        </div>
        <button
          onClick={openAdd}
          disabled={atLimit}
          title={atLimit ? `Limit reached (${maxProducts}). Upgrade your plan.` : undefined}
        ...
        >
          ...
          {t('vendorProducts.addProduct', 'Add Product')}
        </button>
      </div>
      ...
      {maxProducts !== null && (
        <div className={`flex items-center justify-between px-4 py-3 rounded-xl text-sm border ${
          ...
          <span>
            <strong>{usedProducts}</strong> {t('vendorPlan.of', 'of')} <strong>{maxProducts}</strong> {t('vendorProducts.productsUsed', 'products used')}
            {!atLimit && <span className="ml-1 opacity-70">— {maxProducts - usedProducts} {t('vendorPlan.remaining', 'remaining')}</span>}
      ...
      {activePlan && boostTiers.length > 0 && (
        <p className="text-xs text-slate-400">
          ...
          {t('vendorProducts.boostTip', 'Boost a product to appear on the homepage — choose a tier when clicking Boost')}
        </p>
      )}
        ...
        return (
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
              <h3 className="text-base font-bold text-slate-900 mb-1">{t('vendorProducts.boostProduct', 'Boost Product')}</h3>
                  ...
                  {activeTiers.map(t => (
                    <label key={t.id} className={`flex items-center justify-between gap-3 p-3 rounded-xl border-2 cursor-pointer transition-colors ${
                      ...
                      <div className="flex items-center gap-2">
                        <input type="radio" name="boost_tier_p" className="accent-amber-500" checked={selectedTier === t.id} onChange={() => { setSelectedTier(t.id); setBoostBudget('') }} />
                        <div>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          <h3 className="text-sm font-semibold text-slate-900 mb-4">{editing ? t('vendorProducts.editProduct', 'Edit Product') : t('vendorProducts.newProduct', 'New Product')}</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <label className="flex flex-col gap-1 text-xs font-medium text-slate-600 sm:col-span-2">
              {t('common.name')} (EN)
              <input className="border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Product name" />
            </label>
            <label className="flex flex-col gap-1 text-xs font-medium text-slate-600 sm:col-span-2">
              {t('common.name')} (IT)
            ...
            <label className="flex flex-col gap-1 text-xs font-medium text-slate-600">
              ...
              <input type="number" className="border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300" value={form.price} onChange={e => setForm(f => ({ ...f, price: e.target.value }))} />
            </label>
            <label className="flex flex-col gap-1 text-xs font-medium text-slate-600">
              {t('vendorProducts.stock', 'Stock')}
            ...
            <label className="flex flex-col gap-1 text-xs font-medium text-slate-600 sm:col-span-2">
              ...
              <textarea className="border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 resize-y min-h-[64px]" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
            </label>
            <label className="flex flex-col gap-1 text-xs font-medium text-slate-600 sm:col-span-2">
          ...
          <div className="flex gap-2 mt-4">
            ...
            <button onClick={handleSave} disabled={saving} className="flex-[2] py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 text-white text-sm font-semibold rounded-lg border-0 cursor-pointer transition-colors">
              {saving ? t('common.saving', 'Saving…') : editing ? t('vendorProducts.updateProduct', 'Update Product') : t('vendorProducts.createProduct', 'Create Product')}
            </button>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center text-slate-400 text-sm">{t('vendorProducts.noProducts', 'No products yet. Click "Add Product" to get started.')}</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-collapse">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  {[t('common.name'), t('vendorProducts.category', 'Category'), t('vendorServices.price', 'Price'), t('vendorProducts.stock', 'Stock'), t('vendorProducts.boost', 'Boost'), t('vendorBookings.status'), t('common.actions')].map(h => (
              ...
              <tbody className="divide-y divide-slate-100">
                {products.map(p => {
                  const isActiveBoosted = !!p.is_boosted
                  const budgetLeft = isActiveBoosted ? Math.max(0, (parseFloat(p.boost_budget) || 0) - (parseFloat(p.boost_budget_spent) || 0)) : 0
                  return (
                    <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          {p.image_url ? (
                            <img src={resolveImageUrl(p.image_url)} alt={getLocalizedField(p, 'name', i18n.language)} className="w-9 h-9 rounded-lg object-cover border border-slate-200 shrink-0" onError={e => { e.target.style.display = 'none' }} />
                          ) : (
                            <div className="w-9 h-9 rounded-lg bg-slate-100 border border-slate-200 shrink-0" />
                          )}
                          <span className="font-semibold text-slate-900">{getLocalizedField(p, 'name', i18n.language)}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-slate-500">{p.product_category?.name || 'Uncategorized'}</td>
                      <td className="px-4 py-3 font-bold text-emerald-600">€{parseFloat(p.price).toFixed(2)}</td>
                      <td className="px-4 py-3 text-slate-500">{p.stock}</td>
                      <td className="px-4 py-3">
                        {isActiveBoosted ? (
                          <span className="inline-flex items-center gap-1 text-xs font-semibold bg-amber-100 text-amber-700 px-2.5 py-1 rounded-full" title={`${p.boost_clicks || 0} clicks · €${budgetLeft.toFixed(2)} left`}>
                            <BoostIcon /> Boosted · €{budgetLeft.toFixed(2)}
                          </span>
                        ) : (
                          <button
                            onClick={() => openBoostModal(p)}
                            title="Boost this product to appear on homepage"
                      ...
                      <td className="px-4 py-3">
                        <button
                          ...
                          className={`text-xs font-semibold px-3 py-1 rounded-full border-0 cursor-pointer transition-colors ${p.is_active ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
                        >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          <button onClick={() => openEdit(p)} title="Edit" className="p-1.5 rounded-lg bg-violet-50 text-violet-600 hover:bg-violet-100 border-0 cursor-pointer transition-colors"><EditIcon /></button>
                          <button onClick={() => handleDelete(p.id)} title="Delete" className="p-1.5 rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-100 border-0 cursor-pointer transition-colors"><TrashIcon /></button>
                        </div>
                      </td>
                    </tr>
                  )
