...
export default function VendorChatPage() {
  ...
  const loadConversations = ({ silent = false } = {}) => {
    ...
    api.get('/vendor/chat/conversations')
      .then(r => {
  ...
  const loadMessages = (id) => {
    setLoadingMsgs(true)
    api.get(`/vendor/chat/conversations/${id}/messages`)
      .then(r => {
  ...
  const selectConversation = (conv) => {
    ...
    navigate(`/vendor/chat/${conv.id}`, { replace: true })
  }
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      const r = await api.post(`/vendor/chat/conversations/${activeId}/messages`, payload)
  ...
  const handleCloseChat = () => {
    ...
    setImagePreview(null)
    setPendingImageUrl(null)
    navigate('/vendor/chat', { replace: true })
  }
  ...
  const handleClearChat = async () => {
    ...
    setClearing(true)
    try {
      await api.delete(`/vendor/chat/conversations/${activeId}/messages`)
      setMessages([])
      loadConversations()
  ...
  return (
    <div style={{ display: 'flex', flex: 1, minHeight: 0, background: '#f8fafc', borderRadius: 12, overflow: 'hidden', border: '1px solid #e5e7eb' }}>
      {/* Sidebar */}
      <aside style={{ width: 300, flexShrink: 0, minHeight: 0, background: '#fff', borderRight: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '18px 18px 12px', borderBottom: '1px solid #f3f4f6' }}>
          <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: '#111827' }}>Customer Chats</h2>
          <p style={{ margin: '3px 0 0', fontSize: 13, color: '#6b7280' }}>Respond to customer messages</p>
        </div>
        <div className="chat-scroll" style={{ flex: 1, minHeight: 0, overflowY: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {loadingConvs ? (
            <div style={{ padding: 24, color: '#9ca3af', fontSize: 14, textAlign: 'center' }}>Loading…</div>
          ) : conversations.length === 0 ? (
            <div style={{ padding: 32, textAlign: 'center' }}>
              <div style={{ fontSize: 36, marginBottom: 10 }}>💬</div>
              <p style={{ fontSize: 14, color: '#6b7280', margin: 0 }}>No conversations yet.</p>
              <p style={{ fontSize: 13, color: '#9ca3af', marginTop: 6 }}>Customers who message you will appear here.</p>
            </div>
            ...
            <button key={conv.id} onClick={() => selectConversation(conv)} style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', padding: '12px 16px', background: conv.id === activeId ? '#eef2ff' : 'transparent', borderLeft: conv.id === activeId ? '3px solid #4f46e5' : '3px solid transparent', border: 'none', borderBottom: '1px solid #f3f4f6', cursor: 'pointer', textAlign: 'left', transition: 'background 0.15s' }}>
              <div style={{ width: 40, height: 40, borderRadius: '50%', background: '#6366f1', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 15, flexShrink: 0 }}>
                {conv.user?.name?.[0]?.toUpperCase() ?? 'U'}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 14, fontWeight: 600, color: '#111827', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{conv.user?.name ?? 'Customer'}</span>
                ...
                <div style={{ marginTop: 2, display: 'flex', alignItems: 'center', gap: 8 }}>
                  <p style={{ margin: 0, flex: 1, minWidth: 0, fontSize: 12, color: '#6b7280', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {conv.last_message ? <>{conv.last_message.sender_type === 'vendor' ? 'You: ' : ''}{conv.last_message.image_url && !conv.last_message.body ? '📷 Image' : conv.last_message.body}</> : 'New conversation'}
                  </p>
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {!activeId ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
            ...
            <h3 style={{ margin: 0, fontSize: 17, color: '#374151', fontWeight: 600 }}>Select a conversation</h3>
            <p style={{ margin: 0, fontSize: 14, color: '#9ca3af' }}>Choose a customer to start replying</p>
          </div>
        ) : (
          <>
            {/* Header */}
            <div style={{ background: '#fff', borderBottom: '1px solid #e5e7eb', padding: '14px 22px', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
              <div style={{ width: 38, height: 38, borderRadius: '50%', background: '#6366f1', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 15, flexShrink: 0 }}>
                {activeConv?.user?.name?.[0]?.toUpperCase() ?? 'U'}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: 15, color: '#111827' }}>{activeConv?.user?.name ?? 'Customer'}</div>
                <div style={{ fontSize: 12, color: '#10b981' }}>Customer</div>
              </div>
              ...
              <div ref={menuRef} style={{ position: 'relative' }}>
                <button
                  ...
                  style={{ width: 34, height: 34, borderRadius: '50%', background: menuOpen ? '#f3f4f6' : 'transparent', border: '1px solid transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.15s' }}
                  title="Options"
                >
            ...
            {/* Messages */}
            <div className="chat-scroll" style={{ flex: 1, minHeight: 0, overflowY: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none', padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 10 }}>
              {loadingMsgs ? (
                <div style={{ textAlign: 'center', color: '#9ca3af', fontSize: 14 }}>Loading messages…</div>
              ) : messages.length === 0 ? (
                <div style={{ textAlign: 'center', color: '#9ca3af', fontSize: 14, marginTop: 40 }}>No messages yet.</div>
              ) : messages.map(msg => {
                const isMe = msg.sender_type === 'vendor'
                return (
            ...
            {/* Image preview strip */}
            {imagePreview && (
              <div style={{ background: '#fff', borderTop: '1px solid #f3f4f6', padding: '8px 22px', display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ fontSize: 12, color: '#6b7280' }}>{uploadingImage ? 'Uploading…' : 'Ready to send'}</span>
              </div>
            )}

            {/* Input */}
            <form onSubmit={handleSend} style={{ background: '#fff', borderTop: '1px solid #e5e7eb', padding: '12px 22px', display: 'flex', gap: 10, alignItems: 'center', flexShrink: 0 }}>
              <input ref={fileRef} type="file" accept="image/*" onChange={handleImageSelect} style={{ display: 'none' }} />
              <button
                type="button"
                onClick={() => fileRef.current?.click()}
                title="Send image"
                style={{ width: 38, height: 38, borderRadius: '50%', background: '#f3f4f6', border: '1px solid #e5e7eb', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
              >
